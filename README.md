...
- 选手档案：姓名搜索疑似身份、人工勾选合并、展示时间线 / 汇总 / 对手明细（不持久化合并）
- 数据经 SQLite（可切 MySQL）存储，查询页通过 REST API 分页拉取
...
- 默认隐藏 BYE  
- 组别旁展示「对手 N」标识  
- 分页：每页 10 / 20 / 50 / 100；首页、上一页、页码（最多 5 个）、下一页、末尾  
- 选手档案：姓名搜索（≥2 字）→ 勾选 1..N 个身份（含「无账号 ID」警示）→ 查看档案  
- 档案三块：汇总（胜负/奖牌/腰带/赛制/俱乐部）、时间线（按赛事/组别）、对手明细（一场一行，不聚合交手次数）  
